...
if __name__ == "__main__":
    ...
    ax1.title('Error Rate Across Processed Samples with SPC Indicators')
    ax1.grid(True)
    ax1.legend()
    # Plotting states
    ax2.figure(figsize=(10, 3))
    ax2.plot(range(1, len(states) + 1), states, marker='o', linestyle='-', color='b')
    ax2.set_xlabel('Number of processed samples')
    ax2.set_ylabel('State')
    ax2.title('State Across Processed Samples')
    ax2.yticks([0, 1, 2], ['Normal', 'Warning', 'Out of Control'])
    ax2.grid(True)
    plt.tight_layout()
    plt.show()
